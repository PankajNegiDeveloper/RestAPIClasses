...
public class BlogPostServiceImpl implements BlogPostService {
	...
	@Autowired
	private BlogPostServiceImpl(BlogPostRepo blogPostRepo, ModelMapper mapper) {
		super();
	...
	// *****************************************************************
	// Logic for changing DtoToEntity and EntityToDto by Model mapper
...
//		blogPost.setBlogsId(blogPostDto.getBlogId());
	...
	private BlogPostDto mapEntityToDto(BlogPost blogpost) {
		...
		// Converting a BlogPost(entity) to BlogPostDto Model mapper
...
//		List<BlogPost> findAll = blogPostRepo.findAll();
	...
	public BlogPostDto updateBlogPost(BlogPostDto blogPostDto) {
		...
		blogs.setTitle(blogPostDto.getTitle());
		blogs.setDiscription(blogPostDto.getDiscription());
		blogs.setContent(blogPostDto.getContent());

		BlogPost updateblog = blogPostRepo.save(blogs);

		return mapEntityToDto(updateblog);
